...
5. Under **Authorized JavaScript origins**, add every origin sQUIZit is actually served from, e.g. `https://winf-os.github.io` (for the GitHub Pages deploy) and `http://localhost:3000`/whatever port you use for local testing. No path, no trailing slash — origin only. **Also add `https://localhost`** — this is the origin Capacitor's Android WebView actually runs from (its default `androidScheme`/hostname, per `capacitor.config.json`), separate from the GitHub Pages web origin above. Skipping this makes Google Drive backup hang forever on "Google sign-in is still loading" in the native app specifically, since Google Identity Services silently fails to finish initializing outside its registered origins.
...
Uses the `drive.file` scope only — sQUIZit can only see/manage files it created itself (one `quizforge-backup.json`), never the rest of the connected account's Drive.
